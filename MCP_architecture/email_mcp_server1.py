...
def summarize_and_rank_emails(emails_input) -> List[Dict]:
    ...
    if isinstance(emails_input, str):
        ...
        if all(isinstance(item, dict) for item in emails_input):
            emails = []
...
CATEGORIES = [
    "Announcements", "Feedback", "Non-compliance", "System Generated Mails",
...
# Convert LangChain tools to MCP
fetch_tool = StructuredTool.from_function(
    name="FetchEmails",
    func=fetch_new_emails,
    description="Fetch latest emails from the inbox. Returns a dictionary with a key 'emails' containing a list of email strings."
)

process_tool = StructuredTool.from_function(
    name="ProcessEmails",
    func=summarize_and_rank_emails,
    description="Processes a list of email strings. For each email, extracts key info, categorizes, prioritizes, and suggests actions."
)

mcp_tools = [to_fastmcp(fetch_tool), to_fastmcp(process_tool)]

# MCP server
mcp = FastMCP("EmailAssistant", tools=mcp_tools)
